...
import io.ssafy.p.s12b201.techmate.global.error.ErrorResponse;
...
public class ExceptionFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal (
        ...
        } catch (TechMateException e) {
            writeErrorResponse(response, e.getErrorCode(), request.getRequestURL().toString());
        } catch (Exception e) {
            if (e.getCause() instanceof TechMateException) {
                writeErrorResponse(
                        response,
                        ((TechMateException) e.getCause()).getErrorCode(),
